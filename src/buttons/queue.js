...
module.exports = async ({ client, inter, queue }) => { 
    if (!queue || !queue.playing) return inter.reply({ content: `Derzeit läuft doch gar kein Song ❌`, ephemeral: true }),
        console.log(`There was no music playing in the server ${inter.guild.name} when the user ${inter.member.user.username} tried to use the pause button`);

    if (!queue.tracks[0]) return  inter.reply({ content: `Nach dem Song ist Ende Gelände, **du KEK** ❌`, ephemeral: true }),
        console.log(`There was no music in the queue after the current one in the server ${inter.guild.name} when the user ${inter.member.user.username} tried to use the queue button`);
